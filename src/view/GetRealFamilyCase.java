...
public class GetRealFamilyCase extends JInternalFrame {
	...
	public GetRealFamilyCase(int userId) {
        ...
        allChoicesLabel = new JLabel("AllFamilyCases");
        allChoicesLabel.setBounds(149, 119, 123, 34);
        ...
        if(Court.getInstance().getAllLawyers().containsKey(userId)) {
        	if(Court.getInstance().getRealLawyer(userId) instanceof Judge) {
        		for (String i : Court.getInstance().getAllCases().keySet()) {
                	if(Court.getInstance().getRealCase(i) instanceof FamilyCase) {
                		if(Court.getInstance().getRealJudge(userId).getCasesPresided().contains(Court.getInstance().getRealFamilyCase(i))) {
            	 ...
            	 for (String i : Court.getInstance().getAllCases().keySet()) {
                  	if(Court.getInstance().getRealCase(i) instanceof FamilyCase && Court.getInstance().getRealCase(i).getLawyer() != null) {
                 		if(Court.getInstance().getRealFamilyCase(i).getLawyer().getId() == userId) {
                   	       allChoices.addItem(i);
                 		}
                 	}
        ...
        show.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
            	 if(Court.getInstance().getRealFamilyCase((String)allChoices.getSelectedItem()) != null)
                 {
            		 updateDetails();
                 }
            	}
    ...
    private void updateDetails() {
        ...
        String details = Court.getInstance().getRealFamilyCase(selecteId).toString();
