...
public class RemoveEmployee extends JInternalFrame {
	...
	public RemoveEmployee() {
		...
		removeLb = new JLabel("Remove Employee");
		...
		allChoices = new JLabel("All Employees");
		...
		removeBtn.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				Employee emp = Court.getInstance().getRealEmployee((int) choiceBox.getSelectedItem());
				if(Court.getInstance().removeEmployee(emp) == true)
				{
                    JOptionPane.showMessageDialog(null, "Employee removed successfully!", "Success", JOptionPane.INFORMATION_MESSAGE);
                    choiceBox.removeItem(emp.getId());
				}
			 }
		});
	}
