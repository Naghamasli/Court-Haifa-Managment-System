...
public class FindAnewManager extends JInternalFrame {
	...
	public FindAnewManager() {
		...
		date = new JLabel("Please Choose A Department");
		...
		lawyer = new JLabel("The New Manager");
		...
		for(int i: Court.getInstance().getAllDepartments().keySet()) {
			caseBox.addItem(i);
		}
		...
		find.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				Department d = Court.getInstance().getRealDepartment((int)caseBox.getSelectedItem());
				Employee emp = Court.getInstance().AppointANewManager(d);
				if(emp != null) {
		            answerTxt.setText(emp.getId()+" "+emp.getFirstName()+" "+emp.getLastName());
				}else {
					answerTxt.setText("None Found");
				}
			 }
		});
	}
