...
public class AddJudge extends JInternalFrame {

	/**
	 * 
	 */
	private static final long serialVersionUID = 1L;
	private JLabel addJudgeLb;
	private JLabel idLb;
	...
	private JLabel specialzationLb;
	private JComboBox specializationBox;
	private JLabel licenseNumberLb;
	private JTextField licenseTxt;
	private JLabel salaryLb;
	private JTextField salaryTxt;
	private JTextField experienceYearText;
	...
	public AddJudge() {
		...
		addJudgeLb = new JLabel("Judge");
		addJudgeLb.setHorizontalAlignment(SwingConstants.CENTER);
		addJudgeLb.setBounds(239, 27, 317, 34);
		addJudgeLb.setFont(new Font("Book Antiqua", Font.BOLD, 20));
		addJudgeLb.setForeground(new Color(0, 64, 128));
		getContentPane().add(addJudgeLb);
		...
		specialzationLb = new JLabel("specialization");
		specialzationLb.setHorizontalAlignment(SwingConstants.LEFT);
		specialzationLb.setForeground(new Color(17, 80, 136));
		specialzationLb.setFont(new Font("Book Antiqua", Font.PLAIN, 17));
		specialzationLb.setBounds(436, 200, 194, 28);
		getContentPane().add(specialzationLb);
		
		specializationBox = new JComboBox<>(Specialization.values());
		specializationBox.setBounds(598, 205, 135, 22);
		getContentPane().add(specializationBox);
		
		licenseNumberLb = new JLabel("LicenseNumber");
		licenseNumberLb.setHorizontalAlignment(SwingConstants.LEFT);
		licenseNumberLb.setForeground(new Color(17, 80, 136));
		licenseNumberLb.setFont(new Font("Book Antiqua", Font.PLAIN, 17));
		licenseNumberLb.setBounds(436, 251, 194, 28);
		getContentPane().add(licenseNumberLb);
		
		licenseTxt = new JTextField();
		licenseTxt.setForeground(new Color(0, 0, 51));
		licenseTxt.setFont(new Font("Tahoma", Font.PLAIN, 15));
		licenseTxt.setColumns(10);
		licenseTxt.setBounds(598, 251, 136, 26);
		getContentPane().add(licenseTxt);
		...
		JLabel lblExperienceyear = new JLabel("experienceYear");
		lblExperienceyear.setHorizontalAlignment(SwingConstants.LEFT);
		lblExperienceyear.setForeground(new Color(17, 80, 136));
		lblExperienceyear.setFont(new Font("Book Antiqua", Font.PLAIN, 17));
		lblExperienceyear.setBounds(436, 336, 194, 28);
		getContentPane().add(lblExperienceyear);
		
		experienceYearText = new JTextField();
		experienceYearText.setForeground(new Color(0, 0, 51));
		experienceYearText.setFont(new Font("Tahoma", Font.PLAIN, 15));
		experienceYearText.setColumns(10);
		experienceYearText.setBounds(596, 338, 136, 26);
		getContentPane().add(experienceYearText);
		experienceYearText.addKeyListener(new KeyAdapter() {
		    @Override
		    public void keyTyped(KeyEvent e) {
		        char c = e.getKeyChar();
		        if (!Character.isDigit(c)) {
            ...
            public void actionPerformed(ActionEvent e) {
	             ...
	             int ln = 0;
	                ...
	                try {
	                	...
	                	ln = Integer.parseInt(licenseTxt.getText());
	                	salary = Double.parseDouble(salaryTxt.getText());
	                	experience = Integer.parseInt(experienceYearText.getText());
	                	...
	                	if(Court.getInstance().getAllEmployees().containsKey(id) || Court.getInstance().getAllLawyers().containsKey(id) || Court.getInstance().getAllAccuseds().containsKey(id) || Court.getInstance().getAllWitnesses().containsKey(id)) {
	                      ...
	                      Date parsedDate = dateFormat.parse(Birth); // Parse the input
	                      if (parsedDate.after(Court.getInstance().TODAY)) { // Check if it's a future date
	                    ...
	                    try {
							Judge l = new Judge(id,fName,lName, parsedDate, adress, phoneNumber, email, gender,sp, ln,salary, experience );
							Court.getInstance().addJudge(l);
						} catch (InvalidBirthDateException e1) {
							// TODO Auto-generated catch block
							e1.printStackTrace();
						}
	                    JOptionPane.showMessageDialog(null, "Judge added successfully!. The username and password is the ID", "Success", JOptionPane.INFORMATION_MESSAGE);
