...
public class GetRealCriminalcase extends JInternalFrame {
	...
	public GetRealCriminalcase(int id) {
        ...
        allChoicesLabel = new JLabel("AllCriminalCases");
        allChoicesLabel.setBounds(115, 127, 171, 18);
        ...
        if(Court.getInstance().getAllLawyers().containsKey(userId)) {
        	if(Court.getInstance().getRealLawyer(userId) instanceof Judge) {
        		for (String i : Court.getInstance().getAllCases().keySet()) {
                	if(Court.getInstance().getRealCase(i) instanceof CriminalCase) {
                		if(Court.getInstance().getRealJudge(userId).getCasesPresided().contains(Court.getInstance().getRealCriminalCase(i))) {
            	 ...
            	 for (String i : Court.getInstance().getAllCases().keySet()) {
                 	if(Court.getInstance().getRealCase(i) instanceof CriminalCase && Court.getInstance().getRealCase(i).getLawyer() != null) {
                 			if(Court.getInstance().getRealCriminalCase(i).getLawyer().getId() == userId) {
                        	       allChoices.addItem(i);
                      		}
                 	}
        ...
        show.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
            	 if(Court.getInstance().getRealCriminalCase((String)allChoices.getSelectedItem()) != null)
                 {
            		 updateDetails();
                 }
            	}
    ...
    private void updateDetails() {
        ...
        String details = Court.getInstance().getRealCriminalCase(selecteId).toString();
