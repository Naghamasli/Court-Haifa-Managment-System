...
public class RemoveFinancialCase extends JInternalFrame {
	...
	public RemoveFinancialCase() {
		...
		allChoices = new JLabel("All FinancialCase");
		allChoices.setBounds(138, 127, 158, 18);
		...
		{	
	    	if(Court.getInstance().getRealCase(id) instanceof FinancialCase)
			    	choiceBox.addItem(id);
		}
		
		removeBtn.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				FinancialCase c = Court.getInstance().getRealFinancialCase((String) choiceBox.getSelectedItem());
				if(Court.getInstance().removeFinancialCase(c) == true)
				{
                    JOptionPane.showMessageDialog(null, "FinancialCase removed successfully!", "Success", JOptionPane.INFORMATION_MESSAGE);
                    choiceBox.removeItem(c.getCode());
				}
			 }
		});
	}
