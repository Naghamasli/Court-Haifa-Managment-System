...
public class UpdateFinancialCase extends JInternalFrame {
	...
	public UpdateFinancialCase() {
		...
		JLabel lossesAmount = new JLabel("Losses Amount");
		lossesAmount.setHorizontalAlignment(SwingConstants.LEFT);
		lossesAmount.setForeground(new Color(17, 80, 136));
		lossesAmount.setFont(new Font("Book Antiqua", Font.PLAIN, 17));
		lossesAmount.setBounds(459, 134, 194, 28);
		getContentPane().add(lossesAmount);
		...
		amountTxt.setForeground(new Color(0, 0, 51));
		amountTxt.setFont(new Font("Tahoma", Font.PLAIN, 15));
		amountTxt.setColumns(10);
		amountTxt.setBounds(600, 136, 136, 26);
		getContentPane().add(amountTxt);
		
		
		
		JLabel damagedItem = new JLabel("Damaged Item");
		...
		damagedItem.setBounds(459, 173, 194, 28);
		...
		itemTxt.setBounds(600, 175, 136, 26);
		...
		for(String c: Court.getInstance().getAllCases().keySet()) {
			if(Court.getInstance().getRealCase(c) instanceof FinancialCase)
				caseBox.addItem(c);
		}
		...
		showBtn.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
            	FinancialCase c = Court.getInstance().getRealFinancialCase((String)caseBox.getSelectedItem());
            	...
            	itemTxt.setText(c.getDamagedItem());
            	amountTxt.setText(""+c.getLossesAmount());
            }
		...
		doneBtn.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
            	...
            	FinancialCase fc = null;
	                    ...
	                    fc = Court.getInstance().getRealFinancialCase((String) caseBox.getSelectedItem());

            	  
               try {
                    
                    if(amountTxt.getText().isEmpty() || dateTxt.getText().isEmpty() || amountTxt.getText().isEmpty() || itemTxt.getText().isEmpty()) {
                    	throw new FieldsAreNotFullExp();
                    }
                	fc.setAccused(a);
                	fc.setCaseStatus(s);
                	fc.setCaseType(sp);
                	fc.setLossesAmount(amount1);
                	fc.setDamagedItem(item);
                	fc.setLawyer(l);
                	fc.setVerdict(v);
