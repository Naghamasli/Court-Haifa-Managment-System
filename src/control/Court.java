...
public class Court implements Serializable {
	...
	private HashMap<Integer,Appeal>appealApps;
	...
	private Court() {
		...
		this.appealApps = new HashMap<>();
		this.visitorLog = new ArrayList<>();
	}
	
 
    public ArrayList<String[]> getVisitorLog() {
	    return visitorLog;
	}
    public void addVisitor(String visitorName, String department, String judge) {
        visitorLog.add(new String[]{visitorName, department, judge});
    }
	public HashMap<Integer, Appeal> getAppealApps() {
		return appealApps;
	}
	public void setAppealApps(HashMap<Integer, Appeal> appealApps) {
		this.appealApps = appealApps;
	}
	//getInstance
	...
	// add methods
	public boolean addEmployeeToDepartment(Department department , Employee employee) {//done
	...
	public boolean addLawyerToDepartment(Department department , Lawyer lawyer) {//done
	...
	public boolean addJudgeToDepartment(Department department , Judge judge) {//done
	...
	public boolean addDepartment(Department department) {//done
	...
	public boolean addCourtroom(Courtroom courtroom) {//done
	...
	public boolean addLawyer(Lawyer lawyer) {//done
		if(lawyer==null) return false;
		try {
			if(allLawyers.containsKey(lawyer.getId())) 
				throw new ObjectAlreadyExistsException("Lawyer");
	...
	public boolean addJudge(Judge judge) {//done
		if(judge==null) return false;
		try {
			if(allLawyers.containsKey(judge.getId())) 
				throw new ObjectAlreadyExistsException("Judge");
		}catch(ObjectAlreadyExistsException e) {
			MyFileLogWriter.println(e.toString());
			return false;
		}
	...
	public boolean addEmployee(Employee employee) {//done
		if(employee==null) return false;
		try {
			if(allEmployees.containsKey(employee.getId())) 
				throw new ObjectAlreadyExistsException("Employee");
		}catch(ObjectAlreadyExistsException e) {
			MyFileLogWriter.println(e.toString());
			return false;
		}
	...
	public boolean addDocument(Document doc) {//done
	...
	public boolean addVerdict(Verdict verdict) {//done
	...
	public boolean addAccused(Accused accused) {//done
	...
	public boolean addAppeal(Appeal appeal) {//done
	...
	public boolean addTestimony(Testimony testimony) {//done
	...
	public boolean addWitness(Witness witness) {//done
		if(witness==null) return false;

		try {
			if(allWitnesses.containsKey(witness.getId())) 
				throw new ObjectAlreadyExistsException("Witness");
		}catch(ObjectAlreadyExistsException e) {
			MyFileLogWriter.println(e.toString());
			return false;
		}
	...
	public boolean addMeeting(Meeting meet) {//done
	...
	public boolean addCase(Case c) {//done
	...
	public boolean addFinancialCase(FinancialCase c) {//done
		if(c==null) return false;
		if(allCases.containsKey(c.getCode())) return false;
		if(c.getLawyer()!=null) {
			if(!allLawyers.containsKey(c.getLawyer().getId())) allLawyers.put(c.getLawyer().getId(),c.getLawyer());
	...
	public boolean addCriminalCase(CriminalCase c) {//done
		if(c==null) return false;
		if(allCases.containsKey(c.getCode())) return false;
		if(c.getLawyer()!=null) {
			if(!allLawyers.containsKey(c.getLawyer().getId())) allLawyers.put(c.getLawyer().getId(),c.getLawyer());
	...
	public boolean addFamilyCase(FamilyCase c) {//done 
		if(c==null) return false;
		if(allCases.containsKey(c.getCode())) return false;
		if(c.getLawyer()!=null) {
			if(!allLawyers.containsKey(c.getLawyer().getId())) allLawyers.put(c.getLawyer().getId(),c.getLawyer());
	...
	//getReal methods
	public Department getRealDepartment(int number) {//done
		return alldepartments.get(number);
	}
	
	public Courtroom getRealCourtroom(int code) {//done
		return allCourtrooms.get(code);
	}
	
	public Lawyer getRealLawyer(int ID) {//done
		return allLawyers.get(ID);
	}
	
	public Judge getRealJudge(int id) {
	    Lawyer person = getRealLawyer(id); // Ensure this method retrieves the right type

	    if (person instanceof Judge) {
	        return (Judge) person;
	    } else {
	        throw new ClassCastException("Error: Expected a Judge but found " + person.getClass().getSimpleName());
	    }
	}

	public Accused getRealAccused(int ID) {//done
		return allAccuseds.get(ID);
	}
	
	public Employee getRealEmployee(int ID) {//done
		return allEmployees.get(ID);
	}

	public Document getRealDocument(int code) {//done
		return allDocuments.get(code);
	}
	
	public Verdict getRealVerdict(int number) {//done
		return allVerdicts.get(number);
	}
	
	public Testimony getRealTestimony(int code) {//done
		return allTestimonies.get(code);
	}
	
	
	public Witness getRealWitness(int ID) {//done
		return allWitnesses.get(ID);
	}

	public Appeal getRealAppeal(int number) {//done
		return allAppeals.get(number);
	}
	
	public Meeting getRealMeeting(int number) {//done
		return allMeetings.get(number);
	}
	
	public Case getRealCase(String code) {//done
		return allCases.get(code);
	}
	
	public FinancialCase getRealFinancialCase(String code) {//done
		return (FinancialCase) allCases.get(code);
	}
	
	public CriminalCase getRealCriminalCase(String code) {//done
		return (CriminalCase)allCases.get(code);
	}
	
	public FamilyCase getRealFamilyCase(String code) {//done
		return (FamilyCase)allCases.get(code);
	}
	
	
	//remove methods
	public boolean removeDepartment(Department department) {//done
		if(department == null) return false;
		if(!alldepartments.containsKey(department.getNumber())) return false;
	...
	public boolean removeCourtroom(Courtroom courtroom) {//done
		if(courtroom==null) return false;
		if(!allCourtrooms.containsKey(courtroom.getCourtroomNumber())) return false;
		for(Department dep:alldepartments.values())
	...
	public boolean removeLawyer(Lawyer lawyer) {//done
		if(lawyer==null) return false;
	...
	public boolean removeJudge(Judge judge) {//done
		if(judge==null) return false;
	...
	public boolean removeEmployee(Employee employee) {//done
		if(employee==null) return false;
	...
	public boolean removeDocument(Document doc) {//done
		if(doc==null) return false;
		if(!allDocuments.containsKey(doc.getCode())) return false;
		for(Case curCase : allCases.values()) {
			if(doc.getCasee().equals(curCase)) {
	...
	public boolean removeVerdict(Verdict verdict) {//done
	    if (verdict == null) return false;
	    if (!allVerdicts.containsKey(verdict.getVerdictID())) return false;
	...
	public boolean removeAppeal(Appeal appeal) {//done
		if(appeal==null) return false;
		if(!allAppeals.containsKey(appeal.getAppealID())) return false;		
		for(Verdict a : allVerdicts.values()) {
		 if(a.getAppeal() != null) {
			 if(a.getAppeal().equals(appeal)) 
					a.setAppeal(null);
		 }
		}
	...
	public boolean removeTestimony(Testimony testimony) {//done
		if(testimony==null) return false;
		if(!allTestimonies.containsKey(testimony.getTestimonyID())) return false;
	...
	public boolean removeWitness(Witness witness) {//done
	    if (witness == null) return false;
	...
	public boolean removeMeeting(Meeting meet) {//done
		if(meet==null) return false;
		if(!allMeetings.containsKey(meet.getMeetingID())) return false;
		for(Case c:allCases.values()) {
	...
	public boolean removeAccused(Accused accused) {//done
	    if (accused == null) return false;
	...
	public boolean removeCase(Case c) {//done
	    if (c == null) return false;
	    if (!allCases.containsKey(c.getCode())) return false;
	...
	public boolean removeFamilyCase(FamilyCase c) {//done
	    if (c == null) return false;
	    if (!allCases.containsKey(c.getCode())) return false;
	...
	public boolean removeFinancialCase(FinancialCase c) {//done
	    if (c == null) return false;
	    if (!allCases.containsKey(c.getCode())) return false;
	...
	public boolean removeCriminalCase(CriminalCase c) {//done
	    if (c == null) return false;
	    if (!allCases.containsKey(c.getCode())) return false;
	...
	public int howManyCasesBefore(Date date) {//done
		int count=0;
	...
	public int differenceBetweenTheLongestAndShortestCase(Lawyer lawyer) {//done
	...
	public ArrayList<Case> findCasesWithMoreThanThreeTestimoniesFromSameGender(Gender gender) {//done
	...
	public HashMap<Department, Integer> findInActiveCasesCountByDepartment  (){//done
...
//		latestDay = lastestDate +2;
	...
	public Lawyer findTheSuitableLawyer(Case casee) {//done
	...
	public Employee AppointANewManager(Department department) {//done
	...
	public Set<String> findUniqueCrimeToolsByCrimeScene(String crimeScene) {//done
	...
	public List<FamilyCase> findFamilyCasesWithWitnessesFromBothSides(){//done
...
//	            .filter(c -> c instanceof CriminalCase && criminalCase.getCrimeScene().equals(crimeScene))
